...
def run_universe_builder(cfg: UniverseBuilderConfig) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    out_one_line_file = Path(out_one_line)  # no extension, ok
    out_one_line_file.parent.mkdir(parents=True, exist_ok=True)

    with out_one_line_file.open("w", encoding="utf-8") as f:
